# This Makefile is for the Tyco extension to perl.
#
...
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     INC => q[-I../tyco-c/include]
#     LIBS => [q[-L../tyco-c/build -ltyco_c]]
#     NAME => q[Tyco]
#     PREREQ_PM => { JSON::PP=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION => q[0.1.0]
#     XS => { Tyco.xs=>q[Tyco.c] }
#     clean => { FILES=>q[Tyco.c Tyco.bs] }
...
CC = gcc
CCCDLFLAGS = -fPIC
CCDLFLAGS = -Wl,--enable-new-dtags -Wl,-z,relro -Wl,--as-needed -Wl,-z,pack-relative-relocs -Wl,-z,now -specs=/usr/lib/rpm/redhat/redhat-hardened-ld -specs=/usr/lib/rpm/redhat/redhat-hardened-ld-errors -specs=/usr/lib/rpm/redhat/redhat-annobin-cc1 -Wl,--build-id=sha1 
CPPRUN = gcc  -E
DLEXT = so
DLSRC = dl_dlopen.xs
EXE_EXT = 
FULL_AR = /usr/bin/ar
LD = gcc
LDDLFLAGS = -lpthread -shared -Wl,-z,relro -Wl,--as-needed -Wl,-z,pack-relative-relocs -Wl,-z,now -specs=/usr/lib/rpm/redhat/redhat-hardened-ld -specs=/usr/lib/rpm/redhat/redhat-hardened-ld-errors -specs=/usr/lib/rpm/redhat/redhat-annobin-cc1 -Wl,--build-id=sha1  -L/usr/local/lib -fstack-protector-strong
LDFLAGS = -Wl,-z,relro -Wl,--as-needed -Wl,-z,pack-relative-relocs -Wl,-z,now -specs=/usr/lib/rpm/redhat/redhat-hardened-ld -specs=/usr/lib/rpm/redhat/redhat-hardened-ld-errors -specs=/usr/lib/rpm/redhat/redhat-annobin-cc1 -Wl,--build-id=sha1  -fstack-protector-strong -L/usr/local/lib
...
OSVERS = 6.11.0
RANLIB = :
SITELIBEXP = /usr/local/share/perl5/5.42
SITEARCHEXP = /usr/local/lib64/perl5/5.42
SO = so
...
XSPROTOARG = 
XSUBPPDEPS = /usr/share/perl5/ExtUtils/typemap /usr/share/perl5/vendor_perl/ExtUtils$(DFSEP)xsubpp
...
# Tyco might depend on some other libraries:
# See ExtUtils::Liblist for details
#
EXTRALIBS = -L/home/mlauria/typedconfig/tyco-perl/../tyco-c/build -ltyco_c
LDLOADLIBS = -L/home/mlauria/typedconfig/tyco-perl/../tyco-c/build -ltyco_c
BSLOADLIBS = 
LD_RUN_PATH = /home/mlauria/typedconfig/tyco-perl/../tyco-c/build
...
PASTHRU = LIBPERL_A="$(LIBPERL_A)"\
	...
	PASTHRU_INC='-I../tyco-c/include $(PASTHRU_INC)'
...
$(INST_DYNAMIC) : $(OBJECT) $(MYEXTLIB) $(INST_ARCHAUTODIR)$(DFSEP).exists $(EXPORT_LIST) $(PERL_ARCHIVEDEP) $(PERL_ARCHIVE_AFTER) $(INST_DYNAMIC_DEP) 
	...
	$(LD)  $(LDDLFLAGS)  $(LDFROM) $(OTHERLDFLAGS) -o $@ $(MYEXTLIB) \
	  $(PERL_ARCHIVE) $(LDLOADLIBS) -lperl $(PERL_ARCHIVE_AFTER) $(EXPORT_LIST) \
	  $(INST_DYNAMIC_FIX)
	$(CHMOD) $(PERM_RWX) $@
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3SECTION) --perm_rw=$(PERM_RW) -u \
	  lib/Tyco.pm $(INST_MAN3DIR)/Tyco.$(MAN3EXT) 
...
clean :: clean_subdirs
	- $(RM_F) \
	  ...
	  *perl.core MYMETA.json \
	  MYMETA.yml Tyco.base \
	  Tyco.bs Tyco.bso \
	  Tyco.c Tyco.def \
	  Tyco.exp Tyco.o \
	  Tyco_def.old blibdirs.ts \
	  core core.*perl.*.? \
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: Tyco' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Tyco",' >> META_new.json
...
test_static :: subdirs-test_static $(MAP_TARGET)
	PERL_DL_NONLAZY=1 "/home/mlauria/typedconfig/tyco-perl/$(MAP_TARGET)" $(MAP_PERLINC) "-MExtUtils::Command::MM" "-MTest::Harness" "-e" "undef *Test::Harness::Switches; test_harness($(TEST_VERBOSE), '$(INST_LIB)', '$(INST_ARCHLIB)')" $(TEST_FILES)

testdb_static :: static pure_all $(MAP_TARGET)
	PERL_DL_NONLAZY=1 "/home/mlauria/typedconfig/tyco-perl/$(MAP_TARGET)" $(MAP_PERLINC) "-I$(INST_LIB)" "-I$(INST_ARCHLIB)" $(TEST_FILE)
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Tyco" VERSION="0.1.0">' > Tyco.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> Tyco.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR></AUTHOR>' >> Tyco.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Tyco.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="JSON::PP" />' >> Tyco.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-thread-multi-5.42" />' >> Tyco.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Tyco.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Tyco.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Tyco.ppd
